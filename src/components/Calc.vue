<template>
  <div class="calculator">
    ...
    <label
      ...
      клавиатуру</label
    >
    <div v-if="showvk" id="showvk">
      Виртуальная клавиатура
      <button v-for="btn in 10" :key="btn" @click="inputNum(btn - 1)" :name="btn">
        {{ btn - 1 }}
      </button>
      <button @click="eraseOne" name="erase">E</button>
      <br /><br />
      <label><input type="radio" name="check1" value="1" v-model="operch" />Операнд 1</label>
      <label><input type="radio" name="check2" value="2" v-model="operch" />Операнд 2</label>
    </div>
  </div>
</template>
